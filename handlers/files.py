...
async def _generate_text_document(update, context, kind, prompt):
    ...
    try:
        # build_pdf may (on first use) download a font over the network, and PDF/DOCX
        # rendering is CPU work — both are offloaded to a thread so the event loop
        # keeps serving other users while this runs.
        if kind == "pdf":
            file_bytes = await asyncio.to_thread(build_pdf, title, body)
        else:
            file_bytes = await asyncio.to_thread(build_docx, title, body)
